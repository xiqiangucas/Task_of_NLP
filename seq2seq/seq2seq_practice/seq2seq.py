...
class Seq2Seq:
    ...
    def __init__(self, rnn_size, n_layers, X_word2idx, encoder_embedding_dim, Y_word2idx, decoder_embedding_dim,
        ...
        self.X_word2idx = X_word2idx        #词典
        self.encoder_embedding_dim = encoder_embedding_dim
        self.Y_word2idx = Y_word2idx        #词典
    ...
    def add_input_layer(self):
        ...
        self.batch_size = tf.shape(self.X)[0]   #第一维表示batch的大小
    # end method add_input_layer


    def lstm_cell(self, reuse=False):
        # return tf.nn.rnn_cell.LSTMCell(self.rnn_size, initializer=tf.orthogonal_initializer(), reuse=reuse)
        return tf.contrib.rnn.LSTMCell(self.rnn_size, initializer=tf.orthogonal_initializer(), reuse=reuse)
    # end method lstm_cell


    def add_encoder_layer(self):
        encoder_embedding = tf.get_variable('encoder_embedding',
                                            [len(self.X_word2idx), self.encoder_embedding_dim],
                                            tf.float32,
    ...
    def add_decoder_layer(self):
        with tf.variable_scope('decode'):
            decoder_embedding = tf.get_variable('decoder_embedding',
                                                [len(self.Y_word2idx), self.decoder_embedding_dim],
                                                 tf.float32,
                                                tf.random_uniform_initializer(-1.0, 1.0))
        ...
        with tf.variable_scope('decode', reuse=True):
            ...
            predicting_decoder = tf.contrib.seq2seq.BasicDecoder(
                cell = tf.contrib.rnn.MultiRNNCell([self.lstm_cell(reuse=True) for _ in range(self.n_layers)]),
